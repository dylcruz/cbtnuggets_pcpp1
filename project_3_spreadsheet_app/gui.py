...
class SpreadsheetApp:
    ...
    def setup_grid(self, rows, columns):
        for col in range(1, columns + 1):
            label = Label(self.root, text=chr(ord('A') + col - 1))
            label.grid(row=0, column=col, sticky='nesw')

        for row in range(1, rows + 1):
            label = Label(self.root, text=row-1)
        ...
        for row in range(rows):
            for col in range(columns):
                ...
                entry.grid(row=row+1, column=col+1, sticky='nesw')
                entry.insert(0, 0)
...
if __name__ == '__main__':
    app = SpreadsheetApp(rows=8, columns=8)
    app.start()
